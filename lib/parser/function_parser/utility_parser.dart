...
class FpNotParser extends FhirPathParser {
  ...
  List execute(List results, Map<String, dynamic> passed) {
    final toBooleanParser = ToBooleanParser();
    final result = toBooleanParser.execute(results, passed);
    results = result.isEmpty ? [] : [!result.first];
    if (nextParser != null) {
      return nextParser!.execute(results, passed);
    } else {
      return results;
    }
  }
